...
//create the zipcode programs list
...
function mapGeoJSON(args){
	...
	geojson_highlighted_layer = L.geoJson(geojson_data.features.properties.zipcode)
...
function createProgramDashboard(item){
	// clear dashboard
	$('.ProgramDashboard').html(`
	<div style="text-align:center">
		<h2>${item.name1}</h2>
		<h4>Zip Profile</h4>
		<div style="font-size:3em;">${item.name1}</div>
		<p>facilities</p>
	</div>
...
function zoomToFIPS(fips){
	...
	highlight=geojson_highlighted_layer.getLayers().filter(item => item.feature.properties.zipcode === fips)[0];
	...
	properties = zip_tracts.features.filter(item => item.properties.zipcode === fips)[0].properties
	console.log(fips)
	console.log(properties)
